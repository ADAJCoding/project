...
@app.route('/products')
def products():
    products = Product.get_all_by_category()
    return render_template('menu.html', products=products)

@app.route('/order/create', methods=['GET', 'POST'])
def order():
    cakes = Product.get_all_by_category()
    cupcakes = Product.get_all_by_category()
    cookies = Product.get_all_by_category()
...
def destroy(id):
    if request.method == 'GET':
        orders = Product.get_all_by_category()
        return render_template('order.html', orders=orders)
    elif request.method == 'POST':
        order = Product.get_one_product(id)
        if not order:
            return "Order not found"
        try:
...
def show(id):
    if 'user_id' not in session:
        return redirect('/logout')
    return render_template("order.html", order = Product.get_one_wuser(id))
